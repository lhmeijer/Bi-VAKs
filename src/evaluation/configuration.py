...
class BearBConfiguration(object):
    ...
    TRIPLES_PER_UPDATE = 5     # 1, 2, 4
    START_EFFECTIVE_DATE = '2015-01-01T00:00:00+00:00'
    QUERY_TIME = '2015-07-01T00:00:00+00:00'

    SEED = 0    # 0, 1, 2, 3, 4

    _updates = '_'.join((str(SEED), str(TRIPLES_PER_UPDATE), _closeness, _width))
    updates_file_name = os.path.join(os.path.dirname(__file__), '..', '..', 'data', 'BEAR-B-updates', _updates + '.txt')
    statistics_updates_file_name = os.path.join(os.path.dirname(__file__), '..', '..', 'data', 'BEAR-B-updates',
                                                'statistics_' + _updates + '.txt')
    ...
    _ingestion_results = '_'.join((str(SEED), str(TRIPLES_PER_UPDATE), _reference, _repeated, _closeness, _width,
    ...
    _query_results = '_'.join((str(SEED), str(TRIPLES_PER_UPDATE), _reference, _fetching, _repeated,
