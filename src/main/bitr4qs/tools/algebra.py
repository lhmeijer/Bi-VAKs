...
def translate_prologue(p, base, initNs=None, prologue=None):
    if prologue is None:
        prologue = Prologue()
        prologue.base = ""
    if base:
...
def _translate_update(u, prologue):
    if u.name in ('Load', 'Clear', 'Drop', 'Create'):
        ...
        if u.delete:
            u.delete["triples"], u.delete[
                "quads"] = translate_quads(u.delete.quads)
        if u.insert:
            u.insert["triples"], u.insert[
                "quads"] = translate_quads(u.insert.quads)
        u["where"] = u.where
...
def translate_update(q, base=None, initNs=None):
    """
    ...
    for p, u in zip(q.prologue, q.request):
        prologue = translate_prologue(p, base, initNs, prologue)
